...
import { Loader2, Package, X, AlertTriangle, ChevronLeft, ChevronRight, Clock } from "lucide-react";
...
const STATUS_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  Pending: {
    label: PAYMENT_STATUS_LABELS.Pending.label,
    bg: PAYMENT_STATUS_LABELS.Pending.bg,
    text: PAYMENT_STATUS_LABELS.Pending.color,
  },
  Paid: {
    label: PAYMENT_STATUS_LABELS.Paid.label,
    bg: PAYMENT_STATUS_LABELS.Paid.bg,
    text: PAYMENT_STATUS_LABELS.Paid.color,
  },
  Cancelled: {
    label: PAYMENT_STATUS_LABELS.Cancelled.label,
    bg: PAYMENT_STATUS_LABELS.Cancelled.bg,
    text: PAYMENT_STATUS_LABELS.Cancelled.color,
  },
};
  ...
  return (
    <span
      style={{
        ...
        fontSize: 11,
        fontWeight: 600,
        color: expired ? "#94A3B8" : "#F59E0B",
      }}
    >
      <Clock size={11} />
      {expired ? "Đã hết hạn" : `Còn ${formatRemaining(remaining)}`}
    </span>
  );
}
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: 12, color: "#94A3B8", margin: 0 }}>
          ...
          {order.paymentStatus === "Pending" && order.expiresAt && (
            <>
              {" · "}
      ...
      <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
        ...
        {order.paymentStatus === "Pending" ? (
          <Link
            to={`/checkout?payOrderId=${order.orderId}`}
            style={{
              padding: "8px 14px",
              background: "#0E76A8",
              color: "white",
...
export function OrdersPage() {
  ...
  const [tab, setTab] = useState<StatusTab>(justPlacedId ? "Pending" : "All");
  ...
  const [banner, setBanner] = useState<string | null>(justPlacedId ? `Đơn hàng #${justPlacedId} đã được tạo. Vui lòng thanh toán trong vòng 15 phút.` : null);
  ...
  const handleCancel = async (orderId: number) => {
    ...
    try {
      ...
      if (res.success) {
        setOrders((prev) =>
          prev.map((o) => (o.orderId === orderId ? { ...o, paymentStatus: "Cancelled" as const } : o)),
        );
      } else {
  ...
  return (
    <div style={{ background: "#F8FAFC", minHeight: "100vh" }}>
      <Breadcrumb items={[{ label: "Trang chủ", href: "/" }, { label: "Đơn hàng của tôi" }]} maxWidth={900} />
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px" }}>
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            ...
            <p style={{ marginTop: 16, color: "#64748B", fontFamily: "Inter, sans-serif", fontSize: 14 }}>
              Đang tải đơn hàng...
            </p>
          </div>
        ) : error ? (
          ...
          <div style={{ textAlign: "center", padding: "60px 0", background: "white", borderRadius: 16 }}>
            ...
            >
              {tab === "All"
                ...
                : `Không có đơn hàng nào (${tab === "Pending" ? "Chờ thanh toán" : tab === "Paid" ? "Đã thanh toán" : "Đã hủy"})`}
            </p>
            <Link
              to="/vouchers"
              style={{
